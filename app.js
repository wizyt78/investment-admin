...
<button class="mini" onclick='messageUser(${JSON.stringify(u.id)},${JSON.stringify(u.full_name)})'>${tr("messageUser")}</button><button class="mini" onclick='resetUserPassword(${JSON.stringify(u.id)},${JSON.stringify(u.full_name)})'>${tr("resetPassword")}</button><button class="mini" onclick='loginAsUser(${JSON.stringify(u.id)},${JSON.stringify(u.full_name)})'>${tr("loginAsUser")}</button><button class="mini red" onclick='deleteUser(${JSON.stringify(u.id)},${JSON.stringify(u.full_name)})'>${tr("deleteUser")}</button>
...
function resetUserPassword(id,name){
  openModal(tr("resetPassword"),`<form id="resetPasswordForm" class="modal-form"><div class="modal-hero"><span class="eyebrow">${tr("security")}</span><h3>${esc(name)}</h3><p>${tr("newPasswordAdmin")}</p></div><label>${tr("newPasswordAdmin")}<input id="adminNewPassword" type="password" minlength="8" autocomplete="new-password" required></label><label>${tr("confirm")}<input id="adminConfirmPassword" type="password" minlength="8" autocomplete="new-password" required></label><button class="primary">${tr("resetPassword")}</button></form>`);
  $("resetPasswordForm").onsubmit=async e=>{e.preventDefault();try{const p=$("adminNewPassword").value;if(p!==$("adminConfirmPassword").value||p.length<8)throw Error(tr("systemError"));await api("/api/admin/users/"+encodeURIComponent(id)+"/password",{method:"PATCH",body:JSON.stringify({password:p})});closeModal();toast(tr("success"));await loadAudit()}catch(err){toast(err.message)}};
}
function loginAsUser(id,name){
  openModal(tr("loginAsUser"),`<form id="loginAsUserForm" class="modal-form"><div class="modal-hero"><span class="eyebrow">${tr("viewAccount")}</span><h3>${esc(name)}</h3><p>${tr("confirmAction")}</p></div><button class="primary">${tr("loginAsUser")}</button></form>`);
  $("loginAsUserForm").onsubmit=async e=>{e.preventDefault();try{const d=await api("/api/admin/users/"+encodeURIComponent(id)+"/login-as",{method:"POST"});if(d.url)window.open(d.url,"_blank","noopener,noreferrer");else throw Error(tr("systemError"));closeModal()}catch(err){toast(err.message)}};
}
async function deleteUser(id,name){
  if(!confirm(tr("deleteConfirm")+" "+name))return;
  try{await api("/api/admin/users/"+encodeURIComponent(id),{method:"DELETE",body:JSON.stringify({reason:"Admin deactivation"})});toast(tr("userDeactivated"));await loadUsers();await loadAudit()}catch(err){toast(err.message)}
}
