...
async function loadUsers(){
 ...
 <td><div class="mini-actions"><button class="mini" onclick='editUser(${JSON.stringify(u.id)})'>${tr("edit")}</button><button class="mini green" onclick='creditUser(${JSON.stringify(u.id)},${JSON.stringify(u.full_name)})'>${tr("credit")}</button><button class="mini red" onclick='debitUser(${JSON.stringify(u.id)},${JSON.stringify(u.full_name)})'>${tr("debit")}</button><button class="mini" onclick='toggleStatus(${JSON.stringify(u.id)},${JSON.stringify(u.status)})'>${u.status==="active"?tr("suspended"):tr("active")}</button></div></td>
...
async function loadInvestments(){
 ...
 $("investmentList").innerHTML=a.length?table([tr("member"),tr("planName"),tr("amount"),tr("status"),tr("created")],a.map(i=>`<tr><td><b>${esc(i.full_name)}</b><br><small>@${esc(i.username)}</small></td><td>${esc(i.plan_name)}</td><td>${Number(i.amount).toFixed(3)} KWD</td><td><span class="badge ${i.status}">${esc(i.status)}</span></td><td>${esc(i.created_at)}</td></tr>`)):`<div class="empty">${tr("noInvestments")}</div>`;
}
async function loadAudit(){
 const d=await api("/api/admin/audit"),a=d.logs||[];
 $("auditList").innerHTML=a.length?table(["Time","Admin","Action","Target","Amount","Details"],a.map(x=>`<tr><td>${esc(x.created_at)}</td><td>${esc(x.admin_username||"")}</td><td>${esc(x.action)}</td><td>${esc(x.target_username||"")}</td><td>${x.amount==null?"—":Number(x.amount).toFixed(3)+" KWD"}</td><td>${esc(x.details||"")}</td></tr>`)):`<div class="empty">${tr("noAudit")}</div>`;
}
...
function walletModal(id,name,dir){
 openModal(dir==="credit"?tr("credit"):tr("debit"),`<form id="walletForm" class="modal-form"><p><b>${esc(name)}</b></p><label>${tr("amount")}<input id="mAmount" type="number" min="0.001" step="0.001" required></label><label>${tr("currency")}<select id="mCurrency"><option value="KWD">KWD</option><option value="USD">USD</option><option value="EUR">EUR</option></select></label><label>${tr("description")}<input id="mDesc"></label><label>${tr("reference")}<input id="mRef"></label><button class="${dir==="credit"?"primary":"secondary"}">${dir==="credit"?tr("credit"):tr("debit")}</button></form>`);
 $("walletForm").onsubmit=async e=>{e.preventDefault();try{await api("/api/admin/"+dir,{method:"POST",body:JSON.stringify({user_id:id,amount:Number($("mAmount").value),description:$("mDesc").value,reference:$("mRef").value,currency:$("mCurrency").value})});closeModal();toast(tr("success"));await loadUsers();await loadAudit()}catch(err){toast(err.message)}}
}
...
async function reviewFunding(id,status){if(!confirm(tr("confirmAction")))return;try{await api("/api/admin/funding/"+encodeURIComponent(id),{method:"PATCH",body:JSON.stringify({status})});toast(tr("success"));await loadFunding();await loadAudit()}catch(e){toast(e.message)}}
async function reviewWithdrawal(id,status){if(!confirm(tr("confirmAction")))return;try{await api("/api/admin/withdrawals/"+encodeURIComponent(id),{method:"PATCH",body:JSON.stringify({status})});toast(tr("success"));await loadWithdrawals();await loadUsers();await loadAudit()}catch(e){toast(e.message)}}
async function newInvestment(){
 const d=await api("/api/admin/users?limit=200"),users=d.users||[];
 openModal(tr("newInvestment"),`<form id="investmentForm" class="modal-form"><label>${tr("selectUser")}<select id="iUser">${users.filter(u=>u.status==="active"&&Number(u.balance)>0).map(u=>`<option value="${esc(u.id)}">${esc(u.full_name)} (@${esc(u.username)}) — ${Number(u.balance).toFixed(3)} KWD</option>`).join("")}</select></label><label>${tr("planName")}<input id="iPlan" required></label><label>${tr("amount")}<input id="iAmount" type="number" min="0.001" step="0.001" required></label><label>${tr("currency")}<select id="iCurrency"><option>KWD</option><option>USD</option><option>EUR</option></select></label><button class="primary">${tr("createInvestment")}</button></form>`);
 $("investmentForm").onsubmit=async e=>{e.preventDefault();try{await api("/api/admin/investments",{method:"POST",body:JSON.stringify({user_id:$("iUser").value,plan_name:$("iPlan").value.trim(),amount:Number($("iAmount").value),currency:$("iCurrency").value})});closeModal();toast(tr("success"));await loadInvestments();await loadUsers();await loadAudit()}catch(err){toast(err.message)}}
}
async function boot(){
 applyLang();
